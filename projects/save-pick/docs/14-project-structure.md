...
- 최종 갱신: 2026-09-03 (§9.5 로깅 구현 메모 추가 — traceId·마스킹 방식·배치 소요 시간)
...
> **구현 메모(2026-09-03)**
>
> - **요청 추적**: `RequestTraceFilter`가 요청마다 `traceId`를 MDC에 넣고, 처리가 끝나면 `메서드 경로 -> 상태 (소요ms)` 한 줄을 남긴다. 로그 패턴은 `application.yml`의 `logging.pattern.level`에 있어 모든 줄에 `traceId`가 붙는다(요청 밖의 기동·배치 로그는 빈칸). 앞단이 `X-Request-Id`를 주면 이어 쓰고, 응답에도 같은 헤더로 돌려준다. 밖에서 온 값은 영숫자·하이픈만 남기고 64자로 자른다 — 이 값이 모든 로그 줄에 찍히므로 로그 주입을 막아야 한다.
> - **마스킹**: 마스킹 대신 **요청 본문·쿼리스트링·헤더를 아예 기록하지 않는** 쪽을 택했다. 기록하지 않으면 마스킹을 빠뜨릴 자리도 없다. 장애 추적에 필요한 것은 "어느 요청이 몇 ms 걸려 몇 번으로 끝났는가"이지 본문이 아니다. 다만 SQL 로그에는 파라미터가 섞이므로 `org.hibernate.SQL`·`org.hibernate.orm.jdbc.bind` 레벨을 `INFO`로 명시해 운영에서 켜지지 않게 했다.
> - **배치**: 6개 잡의 완료 로그에 소요 시간(ms)을 더했다. 처리 건수는 이전부터 있었다. 실패 건수는 별도 항목으로 두지 않았다 — 현재 잡들은 "대상 N건, 처리 M건"을 남기므로 처리되지 않은 건수가 차이로 드러나고, 개별 실패는 예외 로그로 남는다.
> - **헬스 체크**: `/actuator/health` 요청은 추적 로그에서 제외한다. 로드밸런서가 초 단위로 찔러 로그를 가득 채우는데 추적할 사용자 요청이 아니다.
