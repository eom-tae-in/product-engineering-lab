...
| # | 사항 | 내용 |
|---|---|---|
| I1 | 관리자 계정 생성 경로 | 12번 §5 P7(관리자 가입 API 없음)에 따라 `dev` 프로파일 시드(`DevDataSeeder`)로만 만들었다. 시드가 운영에서 돌지 않아 운영에는 생성 경로가 없었다 → **2026-09-03 해소**: `AdminBootstrapRunner`가 P7이 말한 "운영 스크립트"에 해당한다(12번 §5.1) |
...
## G4 재발행 (2026-09-03 저녁)

구현 단계에서 문서가 계속 바뀌어 발행본과 어긋난 상태였다(발행 시각보다 06·11·12·00의 갱신 시각이 늦었다). `documentation-publisher`가 현재 문서 원문을 정본으로 삼아 셋을 다시 만들었다.

| 산출물 | 이전 | 이번 |
|---|---|---|
| save-pick-overview.pptx | 15슬라이드 | 15슬라이드 |
| save-pick-spec.pdf | 287쪽 | 263쪽 |
| source-map.md | — | 슬라이드 15 + PDF 21장 매핑 |

주요 반영: 서명 알고리즘 HS512 정정(이전 판에는 HS512가 0곳), 06 U1~U5·12 AU-U1~AU-U6 확정표 신규 수록, 11번 응답 필드 추가분, 백엔드 테스트 수 288→303, 보류 4건의 처리 결과(3건 해소·SC-001 미구현 확정). 쪽수가 준 것은 내용 삭제가 아니라 제목 줄이 앞 표에 흡수돼 렌더링되던 결함을 고친 결과다.

### 재발행 중 드러난 문서 정합성 3건 — 같은 날 정리

발행물과 원본 문서가 서로 다른 값을 말하는 상태여서 원본을 고쳤다. 발행물은 이미 정본 값으로 나가 있었다.

| # | 문서 | 문제 | 처리 |
|---|---|---|---|
| 1 | 12-auth.md | §1.3 토큰 구조표와 가정 AU-A2가 `HS256`인데 「확정」 AU-U3만 HS512로 정정돼 문서 내부가 모순 | 본문 두 곳을 HS512로 통일하고 AU-U3 비고에 정정 범위를 적었다 |
| 2 | 17-mvp-scope.md | §5가 P-2·P-4·P-5를 아직 열린 항목으로 적지만 06번 「확정」이 이미 확정한 값들 | 세 항목을 「해소 (2026-09-03)」로 갱신. 남은 것은 P-1·P-3 |
| 3 | 01-service-plan.md | U1(영업시간)·U3(고객 식별 수단)이 미확정으로 남아 있으나 본 문서 「G1 확정값」 #7·#12가 이미 확정 기록 | 「확정 (2026-09-03)」 절로 이관. U2(지표 목표치)만 미확정으로 남긴다 |

## 다음 작업

설계 문서가 요구한 화면·규칙은 모두 구현했거나(28/28 화면, 배치 6/6), 구현하지 않기로 하고 사유를 남겼다. 미확정 값도 전부 확정했다. 남은 것은 다음과 같다.

- **운영 준비 — 종료 (2026-09-03).** 배포 대상과 무관한 준비를 마친 뒤, 실제 배포는 하지 않기로 사용자가 결정했다. 첫 버전 범위 밖이다.

  | 항목 | 결과 |
  |---|---|
  | 관리자 계정 생성 경로 | `AdminBootstrapRunner` (12번 §5.1). I1 해소 |
  | 요청 추적·로그 정책 | `RequestTraceFilter`, 본문 미기록, SQL 레벨 고정 (14번 §9.5 구현 메모) |
  | 헬스 체크 | `/actuator/health`·`liveness`·`readiness` 공개, `show-details` 비노출 |
  | 환경변수 목록 | `backend/.env.example` |
  | 배포 대상·컨테이너 이미지·로그 수집·모니터링 백엔드 | **하지 않음.** 대상이 정해져야 정할 수 있고, 배포 계획이 없다 |

  배포를 하기로 하면 남는 일은 Dockerfile, 플랫폼별 설정, 로그 수집 형식(JSON 구조화 여부), 모니터링 백엔드 연결이다.
- **지표 M2(주문 전환율) 측정 공백.** 상품 상세 조회 이벤트 로깅이 구현돼 있지 않다(17번 §6이 구현 단계 사항으로 예고한 항목). 나머지 M1·M3~M9 8개는 현재 스키마로 산출할 수 있다.
