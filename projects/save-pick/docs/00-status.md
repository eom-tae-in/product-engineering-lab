...
- **운영 준비.** 배포 대상·환경변수(`JWT_SECRET`·DB 접속 정보)·로깅·모니터링은 아직 정하지 않았다. 로컬 실행 방법은 README에 있다. 특히 **운영 프로파일에 관리자 계정을 만들 경로가 없다**(I1) — `dev` 시드가 유일한 생성 수단이라 이대로는 배포해도 관리자 화면에 로그인할 수 없다.
