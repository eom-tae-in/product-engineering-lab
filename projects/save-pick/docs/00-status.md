...
| 영역 | 범위 | 검증 |
|---|---|---|
| 백엔드 | 도메인 7종(계정·매장·상품·재고·장바구니·픽업·주문) + 배치. Java 21 / Spring Boot 3.3 / PostgreSQL 15 / Flyway(19테이블) | 테스트 288개. Testcontainers로 실제 PostgreSQL 사용 |
...
06번이 요구하지만 11번 API가 필요한 정보를 주지 않아 보류한 항목이다. 각각 11번 수정이 선행돼야 한다.

| 화면 | 항목 | 막힌 이유 |
|---|---|---|
| SC-011 · SC-110 | 마감 후 `재고로 되돌리지 않아요` 분기 (BR-019) | API-024·114에 상품 마감 시각이 없어 취소 전 판정 불가 |
| SC-108 | 픽업 시간대 필터 | API-112가 `slotId`를 받지만 그 화면에서 유효한 시간대 목록을 얻을 경로가 없음 |
| SC-106 | 관련 주문 번호 → SC-110 링크 | 원장 응답에 `orderNo`만 있고 `orderId`가 없어 라우트를 만들 수 없음 |
| SC-001 | `상품 N건이 마감돼 목록에서 빠졌어요` | 연속 조회 간 비교가 필요한데 폴링·실시간 갱신 인프라가 없음 |

06번 자체의 미구현 2건(SC-112 수동 노쇼 처리, SC-102 「재고 이력」 메뉴)은 해당 문서에 사유와 되살릴 조건을 주석으로 남겼다.

## 다음 작업
- **deliverables/ 재발행 여부 결정.** PPT·PDF는 2026-08-29 기준이라 그 뒤 문서 변경(06번 구현 메모 2건, 11번 API-114 필드 추가, 본 문서 G5 절)이 반영돼 있지 않다. 재발행하려면 `documentation-publisher`를 다시 실행한다.
- 위 「설계와 어긋나 구현하지 못한 것」 4건은 11번 수정이 선행돼야 한다. 진행하려면 `technical-architect`가 API를 먼저 확정한다.
- 06번 U1·U2·U5와 G2 가정 T2(관리자 세션 수명) 등 미확정 값 결정.
