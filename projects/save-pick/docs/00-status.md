...
- **배포 대상 미정.** 어디에 띄울지가 정해지지 않았다. 이것이 정해져야 컨테이너 이미지·로그 수집 형식(JSON 구조화 여부)·모니터링 백엔드를 정할 수 있다. **사용자 결정이 필요한 유일한 항목이다.**
  - 배포 대상과 무관한 준비는 2026-09-03에 마쳤다: 관리자 계정 생성 경로(I1 → 12번 §5.1), 요청 추적·로그 정책·헬스 프로브(14번 §9.5 구현 메모), 환경변수 목록(`backend/.env.example`).
